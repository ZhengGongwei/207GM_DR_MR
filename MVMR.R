# MVMR 验证3结果 FIG6.pdf(PPT13) 	p值 < 0.05

# g--Collinsella
# s--Collinsella-aerofaciens
# g--Burkholderiales-noname
# f--Burkholderiales-noname         
# s--Burkholderiales-bacterium-1-1-47
# s--Streptococcus-salivarius       
# s--Ruminococcus-torques 
...
se_col = "se",
eaf_col = "eaf",
effect_allele_col = "effect_allele", #alt
other_allele_col = "other_allele", #ref
pval_col = "pval",
...
```shell
cd /share2/pub/zhenggw/zhenggw/MVMR/GM_data

# f__Bacteroidales_noname.tsv g__Collinsella.tsv s__Streptococcus_salivarius.tsv g__Burkholderiales_noname.tsv s__Burkholderiales_bacterium_1_1_47.tsv s__Collinsella_aerofaciens.tsv s__Ruminococcus_torques.tsv 
# for file in *.tsv
# do
# 	awk '$2 !~ /^X/{print}' 
# 	awk -F'\t' 'BEGIN{OFS="\t"; print "SNP\tbeta\tse\teaf\teffect_allele\tother_allele\tpval\tsamplesize"} {
# 	                # 第一段：记录列名所在的列号
# 	                if(NR==1) {
# 	                    for(i=1;i<=NF;i++) {
# 	                        if($i=="id") {id=i}
